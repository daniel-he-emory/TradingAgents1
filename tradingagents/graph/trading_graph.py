...
class TradingAgentsGraph:
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.config["llm_provider"].lower() in ("openai", "ollama", "openrouter"):
            self.deep_thinking_llm = ChatOpenAI(model=self.config["deep_think_llm"], base_url=self.config["backend_url"], model_kwargs={"tools": []})
            self.quick_thinking_llm = ChatOpenAI(model=self.config["quick_think_llm"], base_url=self.config["backend_url"], model_kwargs={"tools": []})
    ...
    def propagate(
        ...
        if deep_think_model:
            if self.config["llm_provider"].lower() in ("openai", "ollama", "openrouter"):
                self.deep_thinking_llm = ChatOpenAI(model=deep_think_model, base_url=self.config["backend_url"], model_kwargs={"tools": []})
            elif self.config["llm_provider"].lower() == "anthropic":
        ...
        if quick_think_model:
            if self.config["llm_provider"].lower() in ("openai", "ollama", "openrouter"):
                self.quick_thinking_llm = ChatOpenAI(model=quick_think_model, base_url=self.config["backend_url"], model_kwargs={"tools": []})
            elif self.config["llm_provider"].lower() == "anthropic":
        ...
        for mem in [self.bull_memory, self.bear_memory, self.trader_memory, self.invest_judge_memory, self.risk_manager_memory]:
            if hasattr(mem, 'situation_collection'):
                mem.situation_collection.delete(where={})
